...
- 出场层面：只使用按层收紧的移动止盈，并由回撤保护、ATR 追踪和交易所保护止损共同执行
...
- `pending_entry_order_id` / `pending_entry_client_oid`：当前加仓单身份，用来把仓位增量精确归因到成交订单
- `last_fill_price`：最近一层订单的实际成交均价，只能从交易所成交记录恢复
- `last_fill_order_id` / `last_fill_amount` / `last_fill_price_source`：最近成交价对应的订单身份、累计成交量与可信来源
- `unresolved_fill_order_id`：已检测到仓位增加、但成交接口暂未返回结果时保留的订单 ID；后续只能重试该订单，不能退回其他同方向成交
- `best_profit_pct`：本轮持仓生命周期内的最高浮盈
- `activated`：移动止盈是否已经激活；当前持仓周期内一旦变为 `true` 就不会再关闭
...
- `protective_stop_profit_lock_ratio=0.75`
- `protective_stop_min_profit_pct=max(2*fee_rate*leverage+0.002, 0.005)`，默认杠杆 3 下等于 `0.005`
- `protective_stop_update_step_pct=0.001`
- `trailing_activation_min_pct=0.005`
- `trailing_activation_max_pct=0.01`
- `trailing_drawdown_min_ratio=0.10`
- `trailing_activation_layer_multipliers=[1.00, 0.90, 0.80, 0.70, 0.60, 0.52, 0.44, 0.37, 0.30]`
- `trailing_drawdown_layer_multipliers=[1.00, 0.92, 0.84, 0.76, 0.68, 0.60, 0.52, 0.44, 0.36]`
...
2. 候选结构价还必须经过统一层间距校验，锚点是已验证的 `last_fill_price`
...
价格粘性不能覆盖安全约束：现有挂单如果不满足相对真实末次成交价的最小间距，会无条件撤销重建。
...
无论价格来自 `STRUCTURE`、`ATR` 还是 `FALLBACK`，策略都会调用 `_enforce_layer_spacing()` 强制拉开距离，并在最终下单前再次校验。

锚点价格 `_layer_anchor_price()` 只接受：

- 从交易所成交记录按订单 ID 聚合、确认过的 `last_fill_price`

持仓均价、当前价格和待成交委托价都不能替代上一层真实成交价。做多要求新价低于上一层并满足间距，做空则要求新价高于上一层并满足间距。

如果成交记录不可用、订单无法唯一归因或部分成交窗口不完整，策略会清空不可信锚点、撤销已有加仓挂单并暂停后续加仓；止盈和止损不受影响。
...
- 撤单接口返回后还会轮询挂单列表，只有目标订单确实消失才算撤单成功
- 重挂前会通过权威 REST 成交历史和精确 `orderId` 终态再次核对；WS 缓存不能作为“零成交”的证明
- 若发现撤单边界成交、订单不是“已取消且零成交”或任一接口不可确认，就禁止替代下单并等待仓位同步
- 所有普通单和条件单都会在首次请求前生成稳定 `clientOid`，同一次网络重试复用该 ID，避免响应丢失时重复创建订单
...
1. 如果之前是空仓，现在有仓了：
   - 按待成交订单 ID 查询真实 fills，并按成交量计算订单成交均价
2. 如果 `pending_layer > layer`：
   - 说明等待中的下一层已成交
   - 把 `layer` 推进到 `pending_layer`
   - 把该订单的实际成交均价写入 `last_fill_price`
   - 清空 `pending_entry_price`
3. 只有真实成交恢复成功后，才更新 `last_known_contracts` 并允许生成下一层计划

同一订单仍有未成交部分时，不会提前部署下一层；后续 partial fill 会继续按同一订单重新计算加权成交均价。

如果首仓或任一加仓刚成交时出现“挂单已消失、持仓暂未变化”的交易所最终一致性窗口，策略会保留 `pending_layer` 和订单 ID。只有精确订单被确认“已取消且零成交”才允许重挂同层；发现成交则等待持仓同步后再推进层级，不会重复补同一层。

如果持仓减少，则认为发生了策略总平过程、保护单成交或外部减仓，并同步真实仓位。

---

## 12. 分层移动止盈逻辑

策略已经取消 TP1 / TP2 分批止盈，只保留整仓移动止盈。收益率均指杠杆后的 ROI。

### 12.1 激活阈值与允许回撤

`_dynamic_tp_values(adx, volatility_pct, layer)` 不再使用固定的“第 1 层 1.2%”表，而是在原有 ADX 与 ATR 波动率逻辑上整体降低目标，再按当前已确认层数继续收紧。

计算顺序：

1. 先算交易成本地板：`fee_floor = 2 * fee_rate * leverage`
2. 激活地板取以下最大值：配置的最小激活值、保护止损最小利润、`fee_floor + 0.1%`
3. 动态基础值取 `max(fee_floor * 1.5, 激活地板)`；旧逻辑使用的成本倍数更高
4. 根据 ADX 与 `ATR / current_price` 乘行情系数，并限制在激活地板与 `trailing_activation_max_pct` 之间
5. 只对“高于激活地板的动态部分”乘层数系数，避免深层阈值低到无法覆盖成本
...
以当前 Gate 配置 `3x`、单边费率 `0.05%` 为例：

- 第 1 层激活值随行情约为 `0.50% ~ 0.975%`，不再固定要求 `1.20%`
- 同一行情进入第 9 层后，激活值约为 `0.50% ~ 0.6425%`
- 第 1 层允许回撤为最高浮盈的 `25% ~ 35%`
- 第 9 层允许回撤约为最高浮盈的 `10% ~ 12.6%`

仓位越深，总风险越大，因此越早激活并保留越多已经出现的利润。配置列表不足总层数时沿用最后一项；即使配置顺序误写，运行时也不会允许深层系数比浅层更宽。

`activated` 是粘性状态：只要本持仓周期曾达到对应动态激活线，之后即使利润回落、指标上下文获取失败或参数上下文变化，移动止盈仍持续有效，直到确认整仓退出并重置策略状态。实际使用的 `active_trailing_drawdown_ratio` 同样会持久化，而且只能变小、不能重新放宽。
...
也就是深层仓位的 ATR 线也会更快收紧。

---

## 13. 单一止盈出口

TP1、TP2、对应的状态字段和分批减仓调用链均已移除。移动止盈激活后，任一退出条件成立都会进入统一的整仓退出流程：

- 最高浮盈回撤达到本层允许比例
- ATR 追踪线被反向穿越
- 交易所服务端保护止损被触发

软件回撤条件一旦命中就必须总平，不再因为当前收益低于 `0.5%` 而拒绝退出。这样即使一次轮询已经错过最佳价，也不会因为利润已经变小而继续持有到亏损。
...
2. 计算应锁定利润：
   ...
   - 至少锁住 `best_profit_pct * (1 - 本层允许回撤比例)`
   - `protective_stop_profit_lock_ratio` 仍作为额外的最低锁定比例
   - 取两者较大值
...
- 第 1 层锁住最高浮盈的至少 `75%`
- 第 9 层按行情锁住最高浮盈的至少约 `87.4% ~ 90%`
- `protective_stop_min_profit_pct=0.5%`

所以软件回撤线和交易所服务端保护单使用同一套利润保留逻辑。
...
即默认新止损相对价格至少改善 `0.1%` 才重挂，比旧值更快跟随利润。

WS 与主轮询并发更新保护止损时，也会在 `action_lock` 内重新读取真实仓位、运行态和当前止损订单身份。旧止损取消失败时会保留本地订单 ID，并禁止创建替代止损，避免远端残留两张保护单。

Gate 返回精确的 `1034 / AUTO_ORDER_NOT_FOUND` 时，表示目标条件单已经成交或被远端删除，撤销的目标状态已经满足。Gate 适配器会把它转换为幂等成功，允许清除本地保护单身份并正常回到 `IDLE`；网络、权限、限流及其他未知错误仍按撤销失败处理，不会放行状态重置。

### 14.4 保护止损与追踪平仓的关系

保护止损是轮询间隙的交易所侧兜底，不会取代软件总平。回撤或 ATR 条件一旦命中，软件直接执行总平；无论当前收益低于 `0.5%`、已经回到零附近还是已经轻微转亏，都不会撤销已经生效的退出决定。
...
`trail_ratio` 不再由 ADX 决定，而是从第 1 层的 `25%` 逐层下降到第 9 层的 `10%`。例如第 9 层最高浮盈达到 `1.0%`，回撤 `0.1` 个百分点就会触发总平。

满足条件时，触发“回撤保护”总平仓。该判断只要求移动止盈已经激活，不再设置最低当前利润门槛。
...
1. 获取 `action_lock` 并设置 `_exit_in_progress`
2. 取消保护止损和未成交开仓单
3. 在锁内重新获取最新持仓，避免使用等待锁之前的旧数量
4. 调用 `close_position()` 平掉最新仓位
5. 严格确认真实仓位为零；API 错误和小残仓都不能算作已平
6. 全撤剩余挂单并确认挂单列表为空，再复核一次仓位
7. 只有“零仓位 + 零挂单”同时确认后才清运行态并回到 `IDLE`
8. 写本地快照并清掉 `_exit_in_progress`
...
6. 盈利达到 ADX、波动率与层数共同算出的动态阈值后永久激活移动止盈；层数越深，激活越早、允许回撤越小
7. 不做 TP1 / TP2 分批减仓，整仓持续受服务端保护止损、回撤保护和 ATR 追踪管理
8. 如果亏损超过硬阈值，立即总平
...
- 它不是“盈利后继续等待高目标”，而是按仓位层数逐级收紧移动止盈
