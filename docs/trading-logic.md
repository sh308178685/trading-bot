...
- `exchange: "bitget"`：支持 Bitget 模拟盘和实盘，行情与账户通道可使用 WebSocket，并以 REST 补充查询。
- `exchange: "weex"`：支持 WEEX USDT 本位合约 V3 实盘接口。公有 WebSocket 订阅 ticker、深度和策略所需 K 线，私有 WebSocket 订阅账户、持仓、成交和订单；断线后自动重连并重新订阅，缓存过期时回退到 REST。下单、撤单、条件单、全仓止损、成交和资金流水均经过 WEEX 适配器转换为策略统一格式。启动时还会读取官方 API 可交易币种列表；配置币种不在列表中时直接拒绝运行。
- WEEX 官方 V3 模拟接口目前只开放余额、下单、持仓和订单历史，缺少撤单、当前委托、条件单和止盈止损接口。为避免出现“能开仓但无法自动管理”的状态，配置 `exchange: "weex"` 且 `sandbox: true` 时程序会拒绝启动。
- Bitget 继续要求单向持仓模式（`one_way_mode`）。WEEX 支持其标准双向持仓模式（`hedge_mode`），但策略仍只管理一个方向：所有订单显式携带 `LONG/SHORT`，启动和运行中一旦发现同一交易对存在多个活动仓位、双方向开仓挂单或与当前仓位相反的增仓挂单，机器人会立即停止。机器人不会自动修改账户级持仓模式。
...
Bitget 只支持 `one_way_mode`。WEEX 允许 `hedge_mode`，因为 WEEX V3 的开仓、平仓、条件单与 TPSL 都显式指定 `positionSide=LONG/SHORT`。这不代表策略会同时做多和做空：机器人仍只管理一个方向，并通过 REST 启动审计、REST/WS 多仓位检测和挂单方向检查防止形成双向敞口。无法确认持仓/挂单，或检测到方向冲突时会拒绝交易；机器人不会自动修改账户级持仓模式。
...
成交和资金流水分别增量保存在 `data/martin-trades.json` 与 `data/martin-ledger.json`，不再把分析范围固定为最近 60 笔。dashboard 的 `roi_pct` 表示自本地权益基线以来、扣除可识别外部划转后的账户收益率；`position_roe_pct` 才是当前持仓 ROE，同时持久化跟踪峰值和最大回撤。交易所接口只提供有限历史窗口，因此首次启用前更早的数据仍需另行导入，之后本地日志会持续积累。
